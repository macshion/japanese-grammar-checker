...
const r = UI_ELEMENTS;
const L = API_CONFIG;
const E = STORAGE_CONFIG;
const S = MESSAGES;
const Y = SYSTEM_PROMPTS;

// Function declaration for function used in compressed code but not defined
function _() {
    // API key prompt function called in the compressed code
    const sidebar = new Sidebar();
    sidebar.showApiKeyPrompt();
}

// 保存历史记录函数 - 在压缩代码中被调用但未定义
function saveToHistory(originalText, correctedText) {
    try {
        chrome.storage.local.get([E.KEYS.HISTORY], result => {
            const history = result[E.KEYS.HISTORY] || [];
            history.unshift({
            ...
            if (history.length > E.MAX_HISTORY_ITEMS) {
                history.length = E.MAX_HISTORY_ITEMS;
            }
            chrome.storage.local.set({ [E.KEYS.HISTORY]: history }, () => {
                // Refresh history display if needed
            });
        });
    } catch (error) {
...
class Sidebar {
    ...
    initializeMessageListeners() {
        ...
        this.storageService.getApiKey().then(apiKey => {
            if (!apiKey) {
    ...
    setEnvironmentLabel() {
        // Add environment label to title if in development mode
        if (IS_DEV_MODE) {
            document.title = `${MESSAGES.DEV_MODE_LABEL} ${document.title}`;
            
            // Add visual indicator for development version
            const devTag = document.createElement('div');
            devTag.className = 'dev-mode-tag';
            devTag.textContent = MESSAGES.DEV_MODE_LABEL;
    ...
    showApiKeyPrompt(callback) {
        ...
        document.getElementById('saveApiKey').addEventListener('click', () => {
            ...
            if (apiKey) {
                this.storageService.setApiKey(apiKey).then(() => {
                    prompt.remove();
...
async function b(){
    const a = document.getElementById(r.HISTORY_LIST);
    document.getElementById(r.HISTORY_CONTAINER);
    
    try {
        chrome.storage.local.get([E.KEYS.HISTORY], v => {
            const d = v[E.KEYS.HISTORY] || [];
            if (d.length === 0) {
                if(a) a.innerHTML = `<li class="text-gray-500">${S.HISTORY_EMPTY}</li>`;
                return;
            }
            
            if(a) {
                a.innerHTML = d.map((n,i) => `
                ...
                a.querySelectorAll(".history-item").forEach(n => {
                    const i = parseInt(n.getAttribute("data-index"));
                    n.addEventListener("click", m => {
                        m.target.closest(".history-actions") || (a.querySelectorAll(".history-item").forEach(e => {e.classList.remove("selected")}), n.classList.add("selected"), document.getElementById(r.INPUT_TEXT).value = d[i].originalText, document.getElementById(r.RESULT).textContent = d[i].correctedText, document.getElementById(r.RESULT_CONTAINER).classList.remove("hidden"));
                    });
                    n.querySelector(".history-copy-btn").addEventListener("click", m => {
                        m.stopPropagation(), navigator.clipboard.writeText(d[i].correctedText);
                        const e = m.target, c = e.textContent;
                        e.textContent = "✓", setTimeout(() => {e.textContent = c}, 1e3);
                    });
                    const I = n.querySelector(".history-view-original-btn");
                    I && I.addEventListener("click", m => {
                        m.stopPropagation();
                        ...
                        u.className = "modal-header";
                        const h = document.createElement("h3");
                        h.textContent = S.ORIGINAL_TEXT_TITLE;
                        const t = document.createElement("button");
                        t.className = "modal-close-btn", t.textContent = "✕", t.addEventListener("click", () => {document.body.removeChild(e)}), u.appendChild(h), u.appendChild(t);
                        const o = document.createElement("div");
                        o.className = "modal-body";
                        const s = document.createElement("pre");
                        s.className = "original-text-modal", s.textContent = d[i].originalText, o.appendChild(s), c.appendChild(u), c.appendChild(o), e.appendChild(c), document.body.appendChild(e), e.addEventListener("click", p => {p.target === e && document.body.removeChild(e)});
                    });
                    const g = n.querySelector(".history-view-explanation-btn");
                    g && g.addEventListener("click", m => {
                        ...
                        u.className = "modal-header";
                        const h = document.createElement("h3");
                        h.textContent = S.EXPLANATION_TITLE;
                        const t = document.createElement("button");
                        t.className = "modal-close-btn", t.textContent = "✕", t.addEventListener("click", () => {document.body.removeChild(e)}), u.appendChild(h), u.appendChild(t);
                        const o = document.createElement("div");
                        o.className = "modal-body";
                        const s = document.createElement("pre");
                        s.className = "explanation-text-modal", s.textContent = d[i].explanation, o.appendChild(s), c.appendChild(u), c.appendChild(o), e.appendChild(c), document.body.appendChild(e), e.addEventListener("click", p => {p.target === e && document.body.removeChild(e)});
                    });
                    const C = n.querySelector(".history-view-full-btn");
                    C && C.addEventListener("click", m => {
                        ...
                        u.className = "modal-header";
                        const h = document.createElement("h3");
                        h.textContent = S.FULL_RESPONSE_TITLE;
                        const t = document.createElement("button");
                        t.className = "modal-close-btn", t.textContent = "✕", t.addEventListener("click", () => {document.body.removeChild(e)}), u.appendChild(h), u.appendChild(t);
                        const o = document.createElement("div");
                        o.className = "modal-body";
                        const s = document.createElement("pre");
                        s.className = "full-response-text", s.textContent = d[i].fullResponse, o.appendChild(s), c.appendChild(u), c.appendChild(o), e.appendChild(c), document.body.appendChild(e), e.addEventListener("click", p => {p.target === e && document.body.removeChild(e)});
                    });
                    n.querySelector(".history-delete-btn").addEventListener("click", m => {
                        if(m.stopPropagation(), confirm("この履歴を削除しますか？")) {
                            const e = [...d];
                            e.splice(i, 1), chrome.storage.local.set({[E.KEYS.HISTORY]:e}, () => {b()});
                        }
                    });
                });
            }
